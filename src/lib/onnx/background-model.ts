...
const MODEL_PATH = `models/modnet-uint8-${MODEL_VERSION}.onnx`;
...
function preprocess(image: ImageBitmap) {
  ...
  // Xenova/MODNet preprocessing: rescale to 0..1 then normalize mean=.5,std=.5.
  for (let i = 0; i < plane; i++) {
    values[i] = (rgba[i * 4] / 255 - 0.5) / 0.5;
    values[plane + i] = (rgba[i * 4 + 1] / 255 - 0.5) / 0.5;
    values[plane * 2 + i] = (rgba[i * 4 + 2] / 255 - 0.5) / 0.5;
  }
...
function postprocessMask(tensor: ort.Tensor, width: number, height: number) {
  ...
  if (dims.length < 2) throw new Error(`Unexpected MODNet output shape: [${dims.join(', ')}]`);
  ...
  if (raw.length < pixels) throw new Error('MODNet output tensor is smaller than expected.');
  ...
  for (let i = 0; i < pixels; i++) {
    const matte = Math.max(0, Math.min(1, raw[i]));
    // Remove faint haze while preserving genuine semi-transparent portrait edges.
    const alpha = Math.round(smoothstep(0.08, 0.92, matte) * 255);
...
export class BackgroundRemovalOnnxModel {
  ...
  async removeBackground(image: ImageBitmap) {
    ...
    if (!inputName || !outputName) throw new Error('MODNet model has an invalid input/output contract.');

    const input = preprocess(image);
    const outputs = await this.session.run({ [inputName]: input });
    const output = outputs[outputName];
    if (!output) throw new Error(`MODNet output '${outputName}' was not returned.`);
