...
function resizeImage(image: ImageBitmap, size: number) {
  ...
  return { data: ctx.getImageData(0, 0, size, size).data, scale, resizedWidth: width, resizedHeight: height };
}
...
function logitsToOriginal(
  ...
  dstCtx.drawImage(
    ...
    0,
    0,
    embedding.originalWidth,
    embedding.originalHeight,
  );
...
export class MobileSamOnnxModel implements InteractiveSegmentationModel {
  ...
  async load() {
    ...
    console.info('SAM decoder contract', {
      inputs: this.decoder.inputNames,
      outputs: this.decoder.outputNames,
    });
  }
  ...
  async encodeImage(image: ImageBitmap): Promise<ImageEmbedding> {
    ...
    const inputName = resolveName(
      this.encoder.inputNames,
      samOnnxConfig.encoderInput,
      ['input_image', 'images', 'image', 'pixel_values'],
      'encoder image input',
    ) ?? this.encoder.inputNames[0];

    const outputName = resolveName(
      this.encoder.outputNames,
      samOnnxConfig.encoderOutput,
    ...
    const input = new ort.Tensor(
      'float32',
      toNchw(data, samOnnxConfig.inputSize),
    ...
    return {
      ...
      inputSize: samOnnxConfig.inputSize,
      scale,
    };
  }
  ...
  async predictMask({
    ...
    for (const point of points) {
      coords.push(point.x * embedding.scale, point.y * embedding.scale);
      labels.push(point.label);
    }

    if (box) {
      coords.push(
        box.x1 * embedding.scale,
        box.y1 * embedding.scale,
        box.x2 * embedding.scale,
        box.y2 * embedding.scale,
      );
      labels.push(2, 3);
    }
    ...
    const embeddingName = resolveName(
      this.decoder.inputNames,
      samOnnxConfig.decoderEmbeddingInput,
      ['image_embeddings', 'image_embedding', 'embeddings'],
      'decoder embedding input',
    );
    const coordsName = resolveName(
      this.decoder.inputNames,
      samOnnxConfig.decoderPointCoordsInput,
      ['point_coords', 'point_coordinates'],
      'decoder point coordinates input',
    );
    const labelsName = resolveName(
      this.decoder.inputNames,
      samOnnxConfig.decoderPointLabelsInput,
      ['point_labels', 'point_label'],
      'decoder point labels input',
    );
    ...
    const maskInputName = resolveName(
      this.decoder.inputNames,
      samOnnxConfig.decoderMaskInput,
      ['mask_input', 'mask_inputs'],
      'decoder mask input',
      false,
    );
    if (maskInputName) {
      feeds[maskInputName] = new ort.Tensor(
        'float32',
    ...
    const hasMaskName = resolveName(
      this.decoder.inputNames,
      samOnnxConfig.decoderHasMaskInput,
      ['has_mask_input', 'has_mask_inputs'],
      'decoder has-mask input',
      false,
    );
    if (hasMaskName) {
      feeds[hasMaskName] = new ort.Tensor('float32', Float32Array.of(0), [1]);
    }

    const originalSizeName = resolveName(
      this.decoder.inputNames,
      samOnnxConfig.decoderOriginalSizeInput,
      ['orig_im_size', 'original_size', 'orig_image_size'],
      'decoder original-size input',
      false,
    );
    if (originalSizeName) {
      feeds[originalSizeName] = new ort.Tensor(
        'float32',
        Float32Array.of(embedding.originalHeight, embedding.originalWidth),
        [2],
      );
    }
    ...
    const maskOutputName = resolveName(
      this.decoder.outputNames,
      samOnnxConfig.decoderMaskOutput,
      ['low_res_masks', 'masks', 'mask'],
      'decoder mask output',
      false,
    ) ?? this.decoder.outputNames[0];
    const scoreOutputName = resolveName(
      this.decoder.outputNames,
      samOnnxConfig.decoderScoreOutput,
      ['iou_predictions', 'scores', 'iou_scores'],
      'decoder score output',
      false,
    );
    ...
    const scores = scoreOutputName && outputs[scoreOutputName]
      ? tensorToFloat32(outputs[scoreOutputName])
      : undefined;
